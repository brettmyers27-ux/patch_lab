...
from core.runtime_compatibility import RUNTIME_FAMILY_ID, validate_runtime_family  # noqa: E402

# Fail closed at build time: without the complete, hash-pinned runtime family the
# datas loop below would silently omit it and produce an app whose Match is
...
for source, destination in (
    ...
    # The parameter predictors are loaded from data/models (core.train and
    # core.delta_model), independent of the runtime family, so they must stay in
    # the frozen app or Match fails with FileNotFoundError.
...
if hf_cache.is_dir():
    ...
    if staged_hf_cache.is_dir():
        # Must be the exact directory core.model_assets resolves inside the
        # frozen app (runtime_data_root()/models/huggingface), or Match reports
        # an incomplete tokenizer cache even though the files are bundled.
        datas.append(
            (str(staged_hf_cache), f"data/runtime/{RUNTIME_FAMILY_ID}/models/huggingface")
        )
